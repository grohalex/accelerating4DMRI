function signal = kc2Res_Sig(kc, initial, thresh)
%returns online respiratory signal from the k-space center (kc)

%online plot
online_plot = 0;%1->display online plot 
                %0->don't display the plot (FASTER)

%k-space center (kc) data with the dims: [nkz nSpokes ncoils]
...
%timing info
t = toc; 
disp(['avg. time per spoke: ',num2str(t/nspokes_online)]); %it makes difference if plotting with or without a plot
signal = Res_Signal;
end